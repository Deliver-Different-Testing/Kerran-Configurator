...
import type { AssignableTargets, AssignTarget, AssignTargetType } from '@/services/tenant_routeService';

// Inline Courier / Agent / NP picker. Mirrors RunViewer's 3-way Assign
// dialog (reimplemented for React): a type toggle + a searchable dropdown
// filtered to the chosen type. NP = an agent with IsNetworkPartner = 1.
...
export function AssignTargetPicker({ targets, value, onChange }: Props) {
  ...
  const list: AssignTarget[] = useMemo(() => {
    if (!targets) return [];
    switch (type) {
  ...
  // Reflect the selected name in the input when the selection / list changes.
  useEffect(() => {
  ...
  const pickType = (t: AssignTargetType) => {
    ...
    setQuery('');
    setOpen(true);
    onChange(null);   // switching list clears the current selection
  };

  const pick = (t: AssignTarget) => {
