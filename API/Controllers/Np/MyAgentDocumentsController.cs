    ...
    INpScopeResolver scopeResolver) : BaseController
{
    ...
    public async Task<IActionResult> Download(int id, [FromQuery] bool inline = false)
    {
        try
        {
            ...
            // inline=true serves Content-Disposition: inline so the NP preview
            // modal can render the PDF in an <iframe> (attachment renders blank).
            // The Download affordance omits the flag and gets attachment.
            if (inline)
            {
