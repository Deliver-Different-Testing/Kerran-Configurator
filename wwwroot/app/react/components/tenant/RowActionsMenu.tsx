...
// Row-overflow (⋯) menu for table rows whose whole body is clickable
// (Recurring Routes Fixes §3/§4). After Fixes 3+4 the Routes/Linehaul Actions
// column collapses to just Copy, so a dedicated column is overkill — this folds
// the leftover action(s) into a kebab popover. The wrapper stops click/keydown
// propagation so opening the menu (or picking an item) never also fires the
// row's onClick → edit-panel open.
//
...
const ROW_HEIGHT = 40;   // approx per-item height, for the flip-up estimate
...
export function RowActionsMenu({ actions, label = 'Row actions' }: { actions: RowAction[]; label?: string }) {
  ...
  return (
    ...
    >
      ...
      {open && coords && (
        <>
          {/* Click-away backdrop. Inside the stopPropagation wrapper, so closing
              the menu never bubbles to the row underneath. */}
